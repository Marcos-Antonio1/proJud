...
let AuthService = class AuthService {
    constructor(techinicianService, jwtService) {
        this.techinicianService = techinicianService;
        this.jwtService = jwtService;
    }
    async validateTechinician(email, password) {
        ...
        if (techinician && await bcrypt.compare(password, techinician.password)) {
            const { password } = techinician, result = __rest(techinician, ["password"]);
            return result;
        }
        return null;
    }
    async login(techinician) {
